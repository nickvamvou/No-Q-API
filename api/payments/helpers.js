...
exports.notifyStakeholdersOfFailedPurchaseAttempt = (job) => async (errorMessage, doneAttempts) => {
  ...
  console.log(`Purchase Failure Attempt Count: ${doneAttempts}`, `Reason: ${errorMessage}`);
...
exports.notifyStakeholdersOfFailedPurchase = (job) => async (errorMessage) => {
  // TODO: For debugging sakes. Remove afterwards!
  console.log('Purchase Failure', `Reason: ${errorMessage}`);
  ...
  const mailOptions = { // TODO: Use appropriate emails
    ...
    template: "forgot-password",
    subject: "Could not create purchase details for this customer",
    context: {} // TODO: Pass appropriate context data to email template, data like `errorMessage`
  };
  ...
  if (error) {
    job.log('Could not notify stakeholders of failure to create purchase -- ', error)
  }
};
