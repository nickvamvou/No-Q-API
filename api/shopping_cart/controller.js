...
module.exports = {
  ...
  */
  deleteActivecarts: async cart_id => {
    ...
    const [queryError, queryResult] = await to(
      dbConn.promiseQuery(deleteActiveCarts, [cart_id])
    );
  ...
  */
  getActiveCartIdAndProductInformation: async (userId, storeId) => {
    ...
    const [queryError, queryResult] = await to(
      dbConn.promiseQuery(getCartIdAndProducts, [userId, storeId])
    );
    //get any possible error
    if (queryError) {
      return queryError;
  ...
  addProductToUsersCartBasedOnBarcode: async (barcode, cartId) => {
    ...
    const [queryError, queryResult] = await to(
      dbConn.promiseQuery(addProductToCartBasedOnBarcode, [barcode, cartId])
    );
    //get any possible error
    if (queryError) {
  ...
  */
  deleteCartFromCartAndFromActive: async (shop_id, user_id) => {
    ...
    const [queryError, queryResult] = await to(
      dbConn.promiseQuery(deleteActiveCart, [shop_id, user_id])
    );
    //get any possible error
    if (queryError) {
      return queryError;
  ...
  */
  cartIsActive: async cartId => {
    ...
    const [queryError, queryResult] = await to(
      dbConn.promiseQuery(checkCartActive, [cartId])
    );
  ...
  */
  createNewCartAndMakeItActive: async (customer_id, store_id) => {
    ...
    const [queryError, queryResult] = await to(
      dbConn.promiseQuery(createNewCart, [customer_id, store_id])
    );
  ...
  */
  ...
  addProductToUsersCart: async (productId, cartId) => {
    ...
    return await new Promise((res, rej) => {
      dbConn.query(addProductToCart, [productId, cartId], (err, result) => {
        if (err) {
          return rej(err);
        } else {
  ...
  */
  checkRFIDScanned: async RFID => {
    ...
    return (product = await new Promise((res, rej) => {
      dbConn.query(getProductBasedOnRFIDReceived, [RFID], (err, result) => {
        if (err) {
          console.log("ERROR");
          return rej(err);
        } else {
  ...
  */
  getCartFromCustomer: async userId => {
    ...
    return (product = await new Promise((res, rej) => {
      dbConn.query(getUsersCart, [userId], (err, result) => {
        if (err) {
          return rej(err);
        } else {
          //the RFID does not exist in db
  ...
  */
  ...
  createCustomerCart: async (customerId, storeId) => {
    ...
    return (cart_id = await new Promise((res, rej) => {
      dbConn.query(createCustomerCart, [customerId, storeId], (err, result) => {
        if (err) {
          return rej(err);
        } else {
          //return the cart id
  ...
  deleteProductFromCart: async (productId, cartId) => {
    ...
    return (result = await new Promise((res, rej) => {
      dbConn.query(removeProductFromCart, [productId, cartId], (err, result) => {
        if (err) {
          return rej(err);
        } else {
  ...
  checkIfVoucherIsRedeemable: async voucherCode => {
    ...
    return (cart_id = await new Promise((res, rej) => {
      dbConn.query(getVoucherIdAndReedemable, [voucherCode], (err, result) => {
        if (err) {
          return rej(err);
        } else {
          //wrong voucher code
  ...
  addVoucherToCartDB: async (voucher_id, cartId) => {
    ...
    return await new Promise((res, rej) => {
      dbConn.query(addVoucherToUserCart, [voucher_id, cartId], (err, result) => {
        if (err) {
          return rej(err);
        } else {
          return res();
