...
const dbConn = require("api/config/db_connection");
const userRoles = require("api/constants/user_roles");
const { SqlError } = require('api/utils/error_constructors');
...
module.exports = {
  ...
  removeStore: (req, res, next) => {
    ...
    //get the store id
    dbConn.query(removeStore, req.params.storeId, (err, result) => {
      if (err) {
        res.status(500).json({
  ...
  getAllStores: (req, res, next) => {
    ...
    dbConn.query(allStores, (err, result) => {
      if (err) {
        res.status(500).json({
          message: err
        });
  ...
  getStoresOfUser: (req, res, next) => {
    ...
    if (authorized) {
      ...
      dbConn.query(storesOfUser, req.params.userId, (err, result) => {
        if (err) {
          res.status(500).json({
            message: err
          });
    ...
    var authorized = module.exports
      ...
      .then(authorized => {
        if (authorized) {
          ...
          dbConn.query(ordersFromStore, req.params.storeId, (err, result) => {
            console.log(result);
            if (err) {
              res.status(500).json({
                message: err
    ...
    module.exports
      ...
      .then(authorized => {
        if (authorized) {
          ...
          dbConn.query(customersFromStore, req.params.storeId, (err, result) => {
            if (err) {
              res.status(500).json({
                message: err
              });
  ...
  addNewProductDetails: async (req, res, next) => {
    ...
    const [queryError, queryResult] = await to(dbConn.promiseQuery('CALL create_product_details(?, ?, ?, ?, ?, ?, ?)', [
      req.body.name,
      req.body.weight,
      req.body.stock,
  ...
  getAllProductDetails: async (req, res, next) => {
    ...
    const [queryError, queryResult] =  await to(
      dbConn.promiseQuery('CALL get_all_product_details_by_store_id(?)', [req.params.storeId])
    );
  ...
  getProductDetails: async ({ params: { productDetailsId, storeId }, userData }, res, next) => {
    ...
    const [queryError, queryResult] = await to(
      dbConn.promiseQuery('CALL get_product_details_by_id(?, ?, ?)', [
        productDetailsId,
        storeId,
        userData.id,
      ])
  ...
  */
  ...
  checkAuthorizationRole: (userId, userIdJwt, role) => {
    if (userId == userIdJwt || role == userRoles.ADMIN) {
      return true;
    } else {
  ...
  checkRetailerOwnsStore: async (userId, storeId) => {
    ...
    return (authorized = await new Promise((resolve, reject) => {
      dbConn.query(getStoreIdBasedOnUserId, [userId, storeId], (err, result) => {
        if (err) {
          resolve(false);
        }
        //empty response
  ...
  checkVoucherExistenceAndRedeemability: async (voucherId, storeId) => {
    ...
    return (voucherId = await new Promise((resolve, reject) => {
      dbConn.query(checkVoucherId, [storeId, voucherId], (err, result) => {
        if (err) {
          reject(0);
        } else {
  ...
  makeCouponUnredeemable: async couponId => {
    ...
    await new Promise((resolve, reject) => {
      dbConn.query(updateCouponRedeemability, [couponId], (err, result) => {
        if (err) {
          reject();
        } else {
          console.log("Starts");
