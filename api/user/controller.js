...
const { cache, dbConnPool, mailer } = require('api/config');
...
module.exports = {
  ...
  */
  ...
  deleteUser: (req, res, next) => {
    ...
    if (authorized) {
      ...
      dbConnPool.query(sql, id, (err, result) => {
        if (err) {
          //user not found
          res.status(400).json({
  ...
  resetPassword: async ({ body: { token: forgotPassToken, newPassword } }, res, next) => {
    ...
    const [jwtError, decodedData] = await to(util.promisify(jwt.verify)(forgotPassToken, process.env.JWT_SALT_KEY));
    ...
    const [cacheRegisterErr, token] = await to(cache.get(`forgot-pass-token-${email}`));
    ...
    const [userQueryError] = await to(dbConnPool.promiseQuery(`CALL change_user_password_by_email(?, ?)`, [email, hashedPass]));
  ...
  */
  ...
  /**
   * Checks if user is authorized to access data based on userRoles.
  ...
  checkAuthorization: (userId, userIdJwt, userRole) => {
    if (userId == userIdJwt || userRole == userRoles.ADMIN) {
      return true;
    } else {
      return false;
    }
  ...
  checkIfVoucherIsRedeemable: async voucherCode => {
    ...
    return (cart_id = await new Promise((res, rej) => {
      dbConnPool.query(getVoucherIdAndReedemable, [voucherCode], (err, result) => {
        if (err) {
          return rej(err);
        } else {
  ...
  getVouchersFromUser: async userId => {
    ...
    return await new Promise((res, rej) => {
      dbConnPool.query(getUserVouchers, [userId], (err, result) => {
        if (err) {
          return rej(err);
        } else {
          return res(result[0]);
  ...
  getUserDetailsFromDb: async userId => {
    ...
    return await new Promise((res, rej) => {
      dbConnPool.query(getUserDetails, [userId], (err, result) => {
        if (err) {
          return rej(err);
        } else {
          console.log(result[0]);
